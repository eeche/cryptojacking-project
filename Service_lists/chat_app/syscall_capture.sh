...
do
    ...
    sudo trace-cmd report > $SAVE_DIR/chat_dashcoin_$n.txt
    echo "시스템 콜 데이터는 $SAVE_DIR/chat_dashcoin_$n.txt 에 저장되었습니다."
    sleep 10
done
...
do
    ...
    sudo trace-cmd report > $SAVE_DIR/chat_doge_$n.txt
    echo "시스템 콜 데이터는 $SAVE_DIR/chat_doge_$n.txt 에 저장되었습니다."
    sleep 10
done
...
do
    ...
    sudo trace-cmd report > $SAVE_DIR/chat_monero_$n.txt
    echo "시스템 콜 데이터는 $SAVE_DIR/chat_monero_$n.txt 에 저장되었습니다."
    sleep 10
done
...
do
    ...
    sudo trace-cmd report > $SAVE_DIR/chat_nimiq_$n.txt
    echo "시스템 콜 데이터는 $SAVE_DIR/chat_nimiq_$n.txt 에 저장되었습니다."
    sleep 10
done
