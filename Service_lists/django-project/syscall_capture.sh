...
docker run -d --name locust_temp -v $(pwd)/locustfile.py:/mnt/locust/locustfile.py locustio/locust -f /mnt/locust/locustfile.py --headless -u 100 -r 10 --host http://nginx
...
do
    ...
    sudo trace-cmd report > $SAVE_DIR/django_dashcoin_$n.txt
    echo "시스템 콜 데이터는 $SAVE_DIR/django_dashcoin_$n.txt 에 저장되었습니다."
    sleep 10
done
...
do
    ...
    sudo trace-cmd report > $SAVE_DIR/django_doge_$n.txt
    echo "시스템 콜 데이터는 $SAVE_DIR/django_doge_$n.txt 에 저장되었습니다."
    sleep 10
done
...
do
    ...
    sudo trace-cmd report > $SAVE_DIR/django_monero_$n.txt
    echo "시스템 콜 데이터는 $SAVE_DIR/django_monero_$n.txt 에 저장되었습니다."
    sleep 10
done
...
do
    ...
    sudo trace-cmd report > $SAVE_DIR/django_nimiq_$n.txt
    echo "시스템 콜 데이터는 $SAVE_DIR/django_nimiq_$n.txt 에 저장되었습니다."
    sleep 10
done
