...
docker run -d --name locust_temp \
  ...
  locustio/locust \
  -f /mnt/locust/locustfile.py --headless -u 100 -r 10 --host http://web
...
do
    ...
    sudo trace-cmd report > $SAVE_DIR/ecommerce_doge_$n.txt
    echo "시스템 콜 데이터는 $SAVE_DIR/ecommerce_doge_$n.txt 에 저장되었습니다."
    sleep 10
done
...
do
    ...
    sudo trace-cmd report > $SAVE_DIR/ecommerce_monero_$n.txt
    echo "시스템 콜 데이터는 $SAVE_DIR/ecommerce_monero_$n.txt 에 저장되었습니다."
    sleep 10
done
...
do
    ...
    sudo trace-cmd report > $SAVE_DIR/ecommerce_nimiq_$n.txt
    echo "시스템 콜 데이터는 $SAVE_DIR/ecommerce_nimiq_$n.txt 에 저장되었습니다."
    sleep 10
done
